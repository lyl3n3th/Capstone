...
function SEnrollment() {
  ...
  const handleDownloadClearance = () => {
    ...
    alert("Clearance downloaded successfully!");
  };

  const handleEnroll = () => {
    if (!eligibilityStatus.grade11Completed) {
      alert("You are not eligible to enroll. Please complete Grade 11 first.");
      return;
    }
    ...
    if (!uploadedFiles["clearance"]) {
      alert("Please upload your clearance before enrolling.");
      return;
    }

    alert("Enrollment submitted successfully! Waiting for registrar approval.");
  };

  const handleDownloadConfirmation = () => {
    if (enrollmentStatus.status !== "Approved") {
      alert("Enrollment confirmation is only available after approval.");
      return;
    }
    ...
    alert("Enrollment confirmation downloaded!");
  };
